...
class RPGNotesAutomator:
    ...
    def __init__(self, auto_config=None):
        ...
        # Armazena configuração automática se fornecida
        self.auto_config = auto_config

        # Inicializa componentes
        self._initialize_components()

        # Interface de usuário (apenas se não for modo automático)
        if not auto_config:
    ...
    def run_automated_workflow(self):
        """
        Executa workflow automatizado para N8N

        Returns:
            bool: True se sucesso, False se erro
        """
        if not self.auto_config:
            print("❌ Modo automático requer configuração")
            return False

        print("🤖 Executando workflow automatizado...")
        print(f"📁 Arquivo: {self.auto_config['input_file']}")
        print(f"🌐 Idioma: {self.auto_config['language']}")
        print(f"🎭 Campanha: {self.auto_config['campaign']}")

        try:
            # Configura cliente Gemini com parâmetros automáticos
            lang_config = {
                'language': self.auto_config['language'],
                'party': {'name': f"Party - {self.auto_config['campaign']}"}
            }

            # Determina template baseado na campanha
            template_mapping = {
                'OOTDL': 'summary-ootdl.txt',
                'Avernus': 'summary-ootdl.txt',  # Usar mesmo template por enquanto
                'Custom': 'summary-ootdl.txt'
            }

            template_name = template_mapping.get(self.auto_config['campaign'], 'summary-ootdl.txt')
            template_path = Path(f"prompts/{self.auto_config['language']}/{template_name}")

            # Atualiza cliente Gemini
            self.gemini_client = GeminiClient(self.config, lang_config, template_path)
            ...
            print(f"❌ Erro no workflow automático: {e}")
            import traceback
            traceback.print_exc()
            return False
